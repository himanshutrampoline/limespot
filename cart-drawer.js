  ...
  class CartDrawer extends SideDrawer {
    ...
    disconnectedCallback() {
      ...
      document.removeEventListener(
        ...
        this.closeDrawerViaEventHandler
      );
    ...
    bindEvents() {
      ...
      if (
        this.cartSummary.classList.contains(
          "cart-drawer--checkout--sticky-true"
        )
      ) {
        this.dcbLoadedHandler =
          this.dcbLoadedHandler || CartDrawer.recalculateCssVarHeights;
        document.addEventListener(
          "shopify:payment_button:loaded",
          this.dcbLoadedHandler
        );
    ...
    open(opener, elementToFocus) {
      ...
      const quickAddDrawer = document.querySelector(
        'quick-add-drawer[aria-hidden="false"]'
      );
    ...
    async refresh(dontRefreshCartItems) {
      try {
        ...
        if (cartItems && !dontRefreshCartItems) {
          ...
          const response = this.getSectionsToRender().map(
    ...
    async renderContents(data, openDrawer = true) {
      if (!this.scriptsLoaded) {
        if (
          !document.querySelector(`script[src="${theme.scripts.cartItems}"]`)
        ) {
        ...
        if (
          !document.querySelector(
            `script[src="${theme.scripts.shippingCalculator}"]`
          ) &&
          this.dataset.shippingCalculator
        ) {
    ...
    getSectionsToRender() {
      return [
        {
          id: "cart-drawer",
          section: this.closest(".shopify-section").id.replace(
            "shopify-section-",
            ""
          ),
