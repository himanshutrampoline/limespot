...
if (!customElements.get("cart-items")) {
  class CartItems extends HTMLElement {
    ...
    init() {
      ...
      this.currentTotalItemCount = Array.from(
      ...
      this.querySelectorAll(".cart-item").forEach((item) => {
        this.currentQuantities[item.dataset.variantId] = Number(
          item.querySelector(".qty-input__input").value
        );
      });
    ...
    async updateQuantity(line, quantity, name) {
      const cartDrawerContent = this.cartDrawer
        ? this.cartDrawer.querySelector(".drawer__content")
        : null;
      const cartDrawerContentScroll = cartDrawerContent
        ? cartDrawerContent.scrollTop
        : 0;
      ...
      document
        .querySelectorAll(
          `.cart-errors, .cart-item__error:not([id="${lineErrorsId}"])`
        )
        .forEach((el) => {
          el.innerHTML = "";
          el.hidden = true;
      ...
      try {
        ...
        if (this.cartDrawer) {
          cartDrawerContent.classList.toggle(
            "drawer__content--flex",
            newTotalQuantity === 0
          );

          if (newTotalQuantity === 0) {
            const recommendations = this.cartDrawer.querySelector(
          ...
          const cartTitle =
            this.closest(".cc-main-cart").querySelector(".js-cart-title");
        ...
        this.getSectionsToRender().forEach((section) => {
          ...
          el.innerHTML = CartItems.getElementHTML(
        ...
        this.updateRecommendations(
          data.item_count > 0 ? data.items[0].product_id : null
        );
        ...
        if (cartDrawerContent) {
          requestAnimationFrame(() => {
            cartDrawerContent.scrollTop = cartDrawerContentScroll;
          });
          setTimeout(() => {
            cartDrawerContent.scrollTop = cartDrawerContentScroll;
          }, 0);
          requestAnimationFrame(() => {
            this.cartDrawer.scrollTop = cartDrawerScroll;
          });
          setTimeout(() => {
            this.cartDrawer.scrollTop = cartDrawerScroll;
          }, 0);
    ...
    async refresh() {
      ...
      try {
        ...
        this.updateRecommendations(
          firstCartItem ? firstCartItem.dataset.productId : null
        );
    ...
    getSectionsToRender() {
      ...
      if (this.cartDrawer) {
        const cartDrawerId = this.cartDrawer
    ...
    enableLoading(line) {
      ...
      const loader = this.querySelector(
        `#cart-item-${line} .cart-item__loader`
      );
    ...
    updateRecommendations(productId) {
      this.recommendations =
        this.recommendations || document.getElementById("cart-recommendations");
...
document.addEventListener("dispatch:cart-items:refresh", async () => {
  ...
  try {
    ...
    cartItemsEl.innerHTML = window.CartItems.getElementHTML(
      data[sectionName],
      "cart-items"
    );
  } catch (err) {
